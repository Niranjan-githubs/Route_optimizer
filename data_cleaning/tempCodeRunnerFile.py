    ...
    def validate_and_split_clusters(self, cluster_labels):
        ...
        if len(noise_students) > 0:
            ...
            for _, student in noise_students.iterrows():
                ...
                walking_distance = 0.0  # Self-cluster, no walking distance
                walking_layer = self.assign_walking_layer(walking_distance)
        ...
        # Separate outliers based on walking distance (>2km)
        if len(assignments_df) > 0:
            outliers_mask = assignments_df['walking_layer'] == 'outlier'
            outliers_df = assignments_df[outliers_mask].copy()
            assignments_df = assignments_df[~outliers_mask].copy()
            
            print(f"📊 Layer distribution:")
            if len(assignments_df) > 0:
                layer_counts = assignments_df['walking_layer'].value_counts()
                for layer, count in layer_counts.items():
                    print(f"   - {layer}: {count} students")
            
            if len(outliers_df) > 0:
                print(f"   - outliers: {len(outliers_df)} students (moved to outliers CSV)")
        else:
